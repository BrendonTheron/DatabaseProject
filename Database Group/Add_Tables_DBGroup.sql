...
CREATE TABLE ACCOMMODATION (
    ACCOM_ID NUMBER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    ACCOM_TYPE VARCHAR2(10) NOT NULL, -- 'FLAT', 'HALL', or 'UNIT'
    ACCOM_REF_ID NUMBER NOT NULL,     -- Flat number, Hall number, or Unit number
    ROOM_NUMBER NUMBER NOT NULL,      -- Room number within the flat/hall/unit
    UNIQUE_ROOM_ID VARCHAR2(20) GENERATED ALWAYS AS (
        ACCOM_TYPE || '-' || LPAD(ACCOM_REF_ID, 3, '0') || '-' || LPAD(ROOM_NUMBER, 2, '0')
    ) VIRTUAL UNIQUE,
    CONSTRAINT chk_accom_type CHECK (ACCOM_TYPE IN ('FLAT', 'HALL', 'UNIT')),
/* this would display as
Hall-001-001 with the range Hall-(001-003)-(001-070)
Flat-001-001 with the range Flat-(001-070)-(001-005)
Unit-001-001 with the range Unit-(001-012)-001*/
    CONSTRAINT unique_accom_ref CHECK (
        (ACCOM_TYPE = 'FLAT' AND ACCOM_REF_ID LIKE '2%' AND LENGTH(ACCOM_REF_ID) = 4) OR
        (ACCOM_TYPE = 'HALL' AND ACCOM_REF_ID LIKE '1%' AND LENGTH(ACCOM_REF_ID) = 3) OR
        (ACCOM_TYPE = 'UNIT' AND ACCOM_REF_ID LIKE '3%' AND LENGTH(ACCOM_REF_ID) = 4)
    )
/* this would display as
Hall-101-001 with the range Hall-(101-103)-(001-070)
Flat-201-001 with the range Flat-(201-070)-(001-005)
Unit-301-001 with the range Unit-(301-012)-001*/
);
...
CREATE TABLE ACCOMMODATION (
    ACCOM_ID NUMBER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    ACCOM_TYPE VARCHAR2(10) NOT NULL, -- 'FLAT', 'HALL', or 'UNIT'
    ACCOM_REF_ID VARCHAR2(5) NOT NULL, -- E.g., '101', '201', '301'
    ROOM_NUMBER NUMBER NOT NULL,      -- Room number within the flat/hall/unit
    UNIQUE_ROOM_ID VARCHAR2(10) GENERATED ALWAYS AS (
        ACCOM_REF_ID || '-' || LPAD(ROOM_NUMBER, 2, '0')
    ) VIRTUAL UNIQUE,
    CONSTRAINT chk_accom_type CHECK (ACCOM_TYPE IN ('FLAT', 'HALL', 'UNIT')),
    CONSTRAINT unique_accom_ref CHECK (
        (ACCOM_TYPE = 'FLAT' AND ACCOM_REF_ID LIKE '2%' AND LENGTH(ACCOM_REF_ID) = 4) OR
